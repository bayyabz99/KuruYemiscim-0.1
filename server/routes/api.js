...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      productsRepo.addImage(parseInt(req.params.id, 10), sizes.medium, req.body.alt || '', req.body.color_name || null);
...
router.delete('/admin/products/images/:imageId', (req, res) => {
  try {
    const fs = require('fs');
    const imageId = parseInt(req.params.imageId, 10);
    const image = productsRepo.findImageById(imageId);
    if (!image) return res.status(404).json({ error: 'Görsel bulunamadı' });

    // Delete from DB
    productsRepo.removeImage(imageId);

    // Delete folder from disk
    if (image.path) {
      const relativeDir = path.dirname(image.path);
      const fullDir = path.join(__dirname, '../../public', relativeDir);
...
router.delete('/admin/organizations/images/:imageId', (req, res) => {
  try {
    const fs = require('fs');
    const imageId = parseInt(req.params.imageId, 10);
    const image = organizationsRepo.findImageById(imageId);
    if (!image) return res.status(404).json({ error: 'Görsel bulunamadı' });

    // Delete from DB
    organizationsRepo.removeImage(imageId);

    // Delete folder containing all sizes from disk
    if (image.path) {
      const relativeDir = path.dirname(image.path);
      const fullDir = path.join(__dirname, '../../public', relativeDir);
      if (fs.existsSync(fullDir)) {
...
router.get('/admin/orders/:id', (req, res) => {
  const order = ordersRepo.findById(parseInt(req.params.id, 10));
