...
function create(data) {
  ...
  if (data.colors && Array.isArray(data.colors)) {
    const insertColor = db.prepare('INSERT INTO product_colors (product_id, name, hex) VALUES (?, ?, ?)');
    data.colors.forEach((c) => {
      if (c.name) {
        insertColor.run(productId, c.name, c.hex || '#000000');
      }
    });
  }
...
function update(id, data) {
  ...
  if (data.colors && Array.isArray(data.colors)) {
    ...
    data.colors.forEach((c) => {
      if (c.name) {
        insertColor.run(id, c.name, c.hex || '#000000');
      }
    });
  }
...
function addImage(productId, path, alt = '', colorName = null) {
  const db = getDb();
  const maxOrder = db
  ...
  return db
    .prepare(
      'INSERT INTO product_images (product_id, path, alt, sort_order, color_name) VALUES (?, ?, ?, ?, ?)'
    )
    .run(productId, path, alt, maxOrder + 1, colorName);
}
...
function findImageById(imageId) {
  const db = getDb();
  return db.prepare('SELECT * FROM product_images WHERE id = ?').get(imageId);
}
